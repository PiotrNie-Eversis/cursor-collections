...
| 🧰 **Reusable skills** | 32 | Topic packages under `.cursor/skills/eversis-*/` — `SKILL.md` on disk; in Agent, use **`eversis-collections` MCP** (`eversis_skills_*`, **`eversis_skill_run_script`** where allowlisted), not a separate Agent Skills path |
...
| 🏠 **MCP (local)** | 1 | **`eversis-collections`** — list/read/validate skills, allowlisted repo scripts, and **`eversis_skill_run_script`** for allowlisted per-skill scripts; build [`mcp/eversis-collections-mcp/`](https://github.com/PiotrNie-Eversis/cursor-collections/tree/main/mcp/eversis-collections-mcp) first (`npm install && npm run build`); not published to npm |
