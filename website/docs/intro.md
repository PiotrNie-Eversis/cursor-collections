...
| 🧰 **Reusable skills** | 32 | Topic packages under `.github/skills/eversis-*/` — `SKILL.md` on disk; in Agent, use **`eversis-collections` MCP** (`eversis_skills_*`, **`eversis_skill_run_script`** where allowlisted), not a separate Agent Skills path |
