...
The Code Reviewer agent enforces tested best practices automatically - the same ones encoded in the framework's production skills (`eversis-technical-context-discovering`, `eversis-code-reviewing`, `eversis-implementing-frontend`). The `eversis-review-codebase` prompt runs a repository-wide pass detecting dead code, duplications, and anti-patterns.
