...
Local **Model Context Protocol** server for the [cursor-collections](https://github.com/PiotrNie-Eversis/cursor-collections) repository. It exposes **`eversis_*` tools** to list, read, and validate the `eversis-*` skill packages under `.github/skills/`, to run a small set of allowlisted root scripts (e.g. `scripts/sync-prompts.mjs`), and to run **allowlisted per-skill scripts** under `.github/skills/eversis-*/scripts/` (see `eversis_skill_run_script`) — not auto-discovered; each entry is code-reviewed in the MCP package.
...
| `eversis_skill_run_script` | Run an allowlisted script under `.github/skills/eversis-*/scripts/` (e.g. `eversis-creating-skills-skill-md-stats`) |
