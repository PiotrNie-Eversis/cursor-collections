...
import { buildSections, readSectionText, updateSectionBody } from "../src/chapters.js";
import { loadDocx, saveDocx } from "../src/docx-io.js";
...
test("buildSections and updateSectionBody (OOXML)", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "eversis-docs-node-"));
