...
**Implement internals:** Engineering Manager **outputs at the start of the first response** the **Implement readiness** block (Flow / Research / Plan / Next gate) — exact four-line contract; optional Reason. Runs an **adequacy checklist** before Research SKIP when `*.research.md` exists (file existence ≠ adequate scope). Then delegates **Research** (Context Engineer) → **Plan** (Architect) → **Implement** (Software / DevOps / E2E / Prompt Engineer by task) **only when artifacts are missing, inadequate, or scope changed**. Pure analysis without `/eversis-implement` is valid; when implement is invoked with analyze intent, stop after research with gate before plan. Attaching `@docs/specs/<issue>/*.plan.md` with **adequate** research skips recreating Ideate artifacts. **Consumer project rules extend** this workflow — they do not override readiness or adequacy. **Pause for human confirmation** after research and after the plan, before large code changes.
...
- **Prompts:** `eversis-ba-docs-planner` → (human gate on `docs-update-plan.md`) → `eversis-ba-docs-writer`. Optionally **`@`** attach `.cursor/rules/eversis-ba-docs-planner.mdc` / `eversis-ba-docs-writer.mdc` for role boundaries (see [Business Manager Docs workflow](../website/docs/workflow/business-manager-docs.md#normative-requirements) — prompt vs rule contract).
- **MCP:** **Atlassian** (Jira + Confluence); **`eversis-collections`** (skills + `.docx` chapter tools — build `mcp/eversis-collections-mcp/` per README).
- **Attachments:** `@website/docs/workflow/business-manager-docs.md`, paths to `summary.md` / target `.docx`; release id and Confluence page title for documentation rules.
