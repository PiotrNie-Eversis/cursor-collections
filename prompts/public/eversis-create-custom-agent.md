...
description: "Create a new custom agent (.agent.md) for Cursor / Copilot Collections upstream. Analyzes existing agents for patterns, guides through design decisions, creates the agent file, and validates against best practices."
...
4. **Create the agent file**: Add `tsh-*.agent.md` under **`.github/agents/`** for upstream. For Cursor-native projects, prefer **`.cursor/rules/eversis-*.mdc`** plus prompts under `prompts/`. Apply the `tsh-creating-agents` skill workflow for structure and validation.
...
<!-- Eversis port; upstream: tsh-create-custom-agent -->
