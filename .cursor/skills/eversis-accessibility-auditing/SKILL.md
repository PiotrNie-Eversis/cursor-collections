---
name: eversis-accessibility-auditing
...
**QA audit workflow** — produces audit reports and business summaries. For **implementation** WCAG patterns during development, use **`eversis-ensuring-accessibility`** (Software Engineer skill), not this skill.
...
> **Note**: Severity levels follow accessibility auditing conventions (aligned with axe-core). For functional bug severity, see `eversis-functional-testing`'s severity matrix which uses Critical/High/Medium/Low.
...
Use chat to resolve ambiguities before and during the audit. Follow these global rules for every question:
