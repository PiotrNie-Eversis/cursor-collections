...
For **primary user-facing public prompts**, consider also creating a **thin `.cursor/commands/eversis-<name>.md`** sibling with the same stem. The command file should contain only two to three sentences instructing the agent to read and execute the canonical `.cursor/prompts/public/` file — no workflow body duplication. This gives users a `/eversis-<name>` entry in the Cursor `/` dropdown as an alternative to `@` attachment. Keep the workflow body exclusively in `.cursor/prompts/public/` to avoid drift.
...
- [ ] If a `.cursor/commands/` sibling was created: verify it only delegates to the canonical prompt (no duplicated workflow steps)
