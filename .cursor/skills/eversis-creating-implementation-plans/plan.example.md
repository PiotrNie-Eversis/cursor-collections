...
- <summary of relevant `.cursor/rules/` or `AGENTS.md` rules — path + key rules>
...
**Verification:** <fast checks from Technical Context → Tech Stack / Testing Patterns>
...
**Clues**: <optional-hints>

#### Task 2.2 - [REUSE] UI Verification of <ui-component-name> by UI Reviewer

**Description**: Run **UI Reviewer** via `eversis-review-ui.md` to verify <ui-component-name> against Figma design. Pass the Figma URL and dev server URL. If verification fails, delegate fix to **Software Engineer** and re-verify (max 5 iterations per component).

**Files:** `src/components/<ui-component-name>.tsx` (reuse)
...
- [ ] Run `<verification command from Technical Context → Testing Patterns>`
- [ ] UI verification passes via UI Reviewer or escalated to user after 5 iterations
...
**Description**: Run **Code Reviewer** via `eversis-review.md` to review the complete implementation. Pass e2e test execution to that agent — do not run e2e tests outside this review.
