...
Always use chat to ask the user where the report should be delivered. Do not assume a destination — the user must choose. If the user provided a flag inline (e.g., `--html`), pre-select that option but still present the question for confirmation.
