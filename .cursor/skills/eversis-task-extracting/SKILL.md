...
- Technical architecture or implementation details (use the **Architect** for that)
- Detailed acceptance criteria from a QA perspective (use the **Context Engineer** for that)
...
- **Existing codebase**: Use `eversis-codebase-analysing` skill to understand what already exists and what needs to be built
...
- **Project baseline** (`docs/context/<project-name>/task-baseline.md`, if present): Review for continuity, overlap, and existing backlog context
...
**Step 3: Review Gate 0 — approve or skip the intent brief**

**Gate 0** validates scope before backlog extraction.

- **Run Gate 0** when materials are ambiguous, conflicting, or scope is unclear. Present `intent-brief.md` in chat; iterate until the user approves.
- **Skip Gate 0** when materials are already unambiguous (e.g. crisp structured requirements with aligned sources). Still save `intent-brief.md` and record in **Gate 0 Approval**: `Skipped — materials unambiguous` with a one-line rationale. Confirm the skip in chat before extraction.

Do not proceed to extraction until Gate 0 is **Approved** or **Skipped** per the rules above.
...
Ask the user in chat to clarify these items. Ask exactly **one question per chat turn**. Each question must clearly identify the specific epic or story it relates to — include the story identifier and title in the question (e.g., "[Epic: User Auth > Story 1.2: User can log in] The transcript mentions SSO but the Figma shows email/password only. Which scope is correct?").

**Step 10: Present task list for user validation**

Present each story to the user individually for validation using **one chat turn per story**. Each turn should include the story's full context: parent epic title, story title, and a brief summary of the acceptance criteria. Ask: "Is this story correct? Should it be split, merged, modified, or removed?"
...
Save `intent-brief.md` and `extracted-tasks.md` to `docs/specs/<workshop-name>/` (or `specifications/<workshop-name>/`).
...
- `eversis-task-analysing` - for business-context exploration, baseline comparison, and intent brief drafting before extraction
