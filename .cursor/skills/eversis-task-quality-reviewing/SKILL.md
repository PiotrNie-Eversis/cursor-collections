---
name: eversis-task-quality-reviewing
...
2. **One suggestion per popup**: Present exactly **one suggestion per chat turn**. Each popup must be self-contained — the user should be able to make a decision without needing context from a previous popup. For each suggestion, show:
...
Save the file to `docs/specs/<workshop-name>/quality-review.md` (or `specifications/<workshop-name>/quality-review.md`).
...
- `eversis-task-extracting` — provides the extracted tasks used as primary input for the quality review
- `eversis-jira-task-formatting` — consumes the updated task list after quality review suggestions are applied