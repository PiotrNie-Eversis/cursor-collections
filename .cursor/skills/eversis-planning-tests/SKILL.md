...
<ac-is-the-contract>
Acceptance Criteria are the contract between product and QA. QA consumes AC — it does not create, complete, or normalize them. If AC are incomplete, redirect to the BA workflow (`@eversis-analyze-materials`) before testing begins.
</ac-is-the-contract>
...
2. Redirect the user: _"The Acceptance Criteria for this task are not test-ready. Please complete them via the BA workflow (`@eversis-analyze-materials`) before proceeding with test planning. Gaps found: [list gaps]."_
...
- **API Testing Gate**: If the feature involves backend interaction, ask the user whether API test scenarios should be included before generating them. Use chat to confirm.
