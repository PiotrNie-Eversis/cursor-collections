...
**Task:** Improve readability of the repository entry README using progressive disclosure and visual workflow mapping (inspired by a product-engineering README benchmark), incorporating stakeholder analysis from the product owner.
...
| **Weak visual anchors** | One lifecycle table (lines 14–20) without phase emojis or WHO/HOW/WHAT breakdown | Does not match product-engineering README benchmark scan pattern |
...
| **Dense prompt table without templates** | Lines 49–60: accurate but not scannable for non-experts | external README benchmark uses Focus / How / Outcome per agent; we have no equivalent at README level |

### What works today (preserve)

- Centered hero with Eversis attribution and product-engineering README benchmark lineage (lines 1–7).
...
| Apply product-engineering README benchmark **principles** adapted to **Cursor** (rules, `@` prompts, `/` commands, MCP) | Copying external benchmark’s 700+ line agent encyclopedia into root README |
...
## Benchmark: product-engineering README benchmark README patterns

Source: product-engineering README benchmark `README.md` (fetched 2026-06-07).

| Pattern | external README implementation | Cursor Collections equivalent |
| ------- | ---------------------- | ----------------------------- |
| **One-liner + tagline** | “Opinionated legacy non-Cursor assistants setup…” + “Product Ideation → Development → Quality” | “Cursor-native product engineering framework” + **Ideate → Implement → Review** |
...
| **Prefix callout** | `eversis-` avoids collisions | `eversis-` prefix callout (already in framework) |
| **Prerequisites** | external assistant Pro, VS Code ≥ 1.109 | **Cursor IDE**, Node for MCP build, `.cursor/rules/` support |
...
**Do not import:** VS Code User Settings, `.github/agents`, `/eversis-*` commands, external license requirement.
...
| **Team lead / EM** | Human gates, Fine → QA handoff, consumer vs vendor setup | Gates mentioned but not visualized like external benchmark flowcharts |
...
- **Product:** Cursor Collections (Eversis) — **Cursor-only**; based on product-engineering README benchmark.
...
```text
1. Hero — title, one-liner, tagline (Ideate → Implement → Review), Eversis + product-engineering README benchmark credit
...
| Emoji overload | Phase emojis only in matrix + flowcharts (match product-engineering README benchmark discipline) |
...
- [x] product-engineering README patterns mapped to Cursor equivalents
