# Research: Align README MCP section with product-engineering README benchmark style

**Task:** Improve [`README.md`](../../../README.md) § **🔌 MCP server configuration** (from line ~259) to follow the structure and tone of product-engineering README benchmark — MCP Server Configuration.
...
The current README MCP block is **informative** (tables, local `eversis-collections` build, verify step) but **does not match the product-engineering README benchmark scan pattern** the stakeholder uses as the benchmark.
...
| **Server usage** | Two tables | **Emoji bullet list** with workflow hooks (`/eversis-implement`) |
| **Sequential Thinking** | One table row | **Dedicated subsection** explaining revise / branch / track |
| **Local framework server** | Prominent (correct for Cursor) | N/A — **keep as Cursor-specific addition**, not in external benchmark |
...
| Adapt external benchmark **section order and micro-templates** to **Cursor** (`.cursor/mcp.json`, Settings → MCP) | Changing committed [`.cursor/mcp.json`](../../../.cursor/mcp.json) |
...
- **Key:** `mcpServers` (Cursor; external benchmark sample uses `servers` in Context7 snippet — use **`mcpServers`** in README JSON).
...
Follow external benchmark order; insert **Cursor-only** blocks without breaking scan flow:
...
### What each MCP is used for
 Emoji bullets (not table) — map to @eversis-* not /eversis-*

> Callout: API keys / OAuth — configure per provider

### Sequential Thinking MCP
 Revise / Branch / Track bullets (from external benchmark)

### Verify
 eversis_skills_list smoke check; disable unused servers
```

**Length target:** ~75–95 lines (vs current ~56). Slight increase for external benchmark parity; remove redundant third-party **table** when bullets cover the same ground.

---

## Draft copy: “What each MCP is used for” (bullets)

Adapt external benchmark bullets to Cursor naming:
...
Mirror external benchmark (add PDF/AWS as optional second line or integrations link):
...
| Risk | Mitigation |
| ---- | ---------- |
| README MCP section longer than external benchmark | Drop third-party table; use bullets only |
...
| Stale external URLs | Reuse URLs already in mcp-setup.md / external README benchmark |
...
- Benchmark: product-engineering README benchmark — MCP Server Configuration
